...
(function(angular){
    "use strict";
    angular.module('app')
        .controller('serviceAdjustListController', ServiceAdjustListController);
    /* @ngInject */
    function ServiceAdjustListController($scope, $rootScope, setting,Restangular,$stateParams,toastr,auth,$state){
        var vm = this;
        vm.list;
        vm.noData = false;
        getServiceAdjustList();
        //取得老人一览
        function getServiceAdjustList() {
            var account = auth.getObject("account");
            vm.credentials={
                "userId":account.userId
            };
            var proc = Restangular.all('reqServiceAdjustList');
        ...
        vm.toDetail = function(item){
            $state.go("app.serviceAdjustDetail", {

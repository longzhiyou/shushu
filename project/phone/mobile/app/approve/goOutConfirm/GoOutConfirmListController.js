...
(function(angular){
    "use strict";
    angular.module('app')
        .controller('goOutConfirmListController', GoOutConfirmListController);
    /* @ngInject */
    function GoOutConfirmListController($scope, $rootScope, setting,Restangular,$stateParams,toastr,auth,$state){
        var vm = this;
        vm.list;
        vm.noData = false;
        getGoOutConfirmList();
        //取得老人一览
        function getGoOutConfirmList() {
            var account = auth.getObject("account");
            vm.credentials={
                "userId":account.userId
            };
            var proc = Restangular.all('reqGoOutConfirmList');
        ...
        vm.toDetail = function(item){
            ...
            $state.go("app.goOutConfirmDetail", {
        ...
        $rootScope.$on("CallGoOutConfirmListMethod",function(){
            getGoOutConfirmList();
        });
    }
